...
const buttonHandler = () => {
    if(modalMessageType === 'success'){
        //do something
        navigation.navigate('RestPassword', { code: code  , email : route.params.email });
    }
...
const handleCodeVerification = async () => {
  ...
  try {
    ...
    const response = await fetch('https://84d6-197-26-59-238.eu.ngrok.io/api/v1/otp/verify',{
      method: 'POST',
      headers: {
...
const resendEmail = async (triggerTimer) => {
  try {
    ...
    console.log('Verifying code for email:', email);
    const response = await fetch('https://84d6-197-26-59-238.eu.ngrok.io/api/v1/forget_password', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    return (
      <MainContainer>
        <KeyboardAvoidingWrapper>    
                ...
                {!verifing && pinReady && <StyledButton onPress={handleCodeVerification}>
                                <ButtonText>
                                    Verifier
                                </ButtonText>
                            </StyledButton>}
